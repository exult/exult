...
void Bwundiai object#(0x490) ()
{
	if (event == STARTED_TALKING) 
	{
		...
		if (!BWUNDIAI->get_item_flag(MET))
			say("@Thou... the one... Gwenno spoke.\" ~You can tell the creature obviously has great trouble with your language.@"); // First words with face shown.
        ...
        converse (0)
			{
			case "name" (remove):
                say("@Bwundiai my name is.@"); // His name was misspelled in his original code!
			...
			case "Gwani" (remove):
				...
				if ((npcNearbyAndVisible(IOLO)) && (npcCanTalk(IOLO)))
					...
					UI_set_conversation_slot(1);
					say("Iolo glances over to you with a glint of sadness in his eyes."); // New content
					UI_remove_npc_face1();
					}
	...
	// Else required to avoid repeating normal conversation.
	// This should avoid the incorrect checks for the Leopard Rug frame 8 and water tile.
	else Bwundiai.original();
}
